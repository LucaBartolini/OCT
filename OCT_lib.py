...
class bscan:
    ...
    def __init__(self, **kwargs):
        ...
        if self.path is None:
            ...
            self.path = get_file(initdir = self.path)
        # in case the given path is relative, turns it into absolute
        ...
        print(
            f'"{self.filename}" loaded. - {self.ascanlength}x{self.width}px, bypass: {self.bypass}\n')
    ...
    def get_level_set(self, mode, **kwargs):
        ...
        elif mode in ['max_intensity', 'max']:
            # this works well, in those relatively common cases in which the surface has the maximum intensity
            levelset = np.zeros(self.raw.shape)
            # all pixels below the maximum are 1, all above are 0
            for Ascan, levelset_column in zip(self.raw.T, levelset.T):
                # it seems this detection works better if we omit a smoothing such as `Ascan = smooth_profile(Ascan, rel_window_size=0.15)`
                levelset_column[Ascan>=max(Ascan)] = 1
            return levelset

                
    def get_profile(self, levelset=None, exclude_top_px=150, smoothed = False, **kwargs):
        """
        ...
        if levelset is None:
            ...
            if mode == 'chan_vese':
                ...
                levelset = self.get_level_set(mode='max')
        ...
        for i, Ascan in enumerate(levelset.T):
            try:
                ...
                profile[i] = None
        if smoothed:
            profile = smooth_profile(profile, rel_window_size=0.1)
        return profile
    ...
    def save_png(self, img=None, profile=None, levelset=None, name=None):
        ...
        if levelset is not None:
            ax.contour(np.flip(levelset, axis=0), [0.5], colors='r', alpha=0.5)
        elif profile is not None:
            ax.plot((self.ascanlength-profile), color='red', alpha=0.5)

        if self.debug:
            print(f"Saving image at: \n{fname}")
        try:
            fig.savefig(fname, bbox_inches='tight', pad_inches=0, dpi=150)
        except FileNotFoundError as e:
            print(f"Can't save at location {fname}. Caught error:\n{e}")
            print(f"\nWill save in {os.getcwd()}")
            fname = os.path.join(os.getcwd(),'Temp.png')
            fig.savefig(fname, bbox_inches='tight', pad_inches=0, dpi=150)
...
def apply_filter(img, **kwargs):
    """
    Applies a custom filter an image, as specified by `mode`
    Depending on the mode, additional parameters are available
    ...
    """
    mode = kwargs.get('mode')
    assert mode is not None, "Please specify a filter mode"
    if mode not in ['bilateral', 'NLM']:
        ...
        img = denoise_nl_means(
            img, h=12*sigma_est, sigma=sigma_est, fast_mode=True, **patch_kw)
...
def asymmetry_factor(profile, peak_cutoff = 0.2):
    ...
    """DEPRECATED: Evaluates the asymmetry of a given profile, returning a value in the range [0,1]
    Please, use `OCT_lib.AF` instead
    
    The profile is a f(x). We can decompose f(x) in a symmetric and an antisymmetric component:
    f(x) = ( f_plus(x) + f_minus(x) )
    where the symmetric component is: f_plus(x) = f(x) + f(-x)
    and the asymmetric component is:  f_minus(x)= f(x) - f(-x)
    ...
    """
    print("WARNING: deprecated. This is a legacy function, still used in some old comparison examples.")
    print("Please, use function `OCT_lib.AF` instead")
    profile_peak = max(profile)
    peak_idx  = int(np.argwhere(profile == profile_peak)[0])
...
def AF(profile, peak_cutoff = 0.2):
    ...
    f_left_norm  = np.sqrt( np.sum(f[:int(len(f)//2)] )**2)
    f_right_norm = np.sqrt( np.sum(f[int(len(f)//2):] )**2) #+1 to exclude the peak
    
    af3 = (f_right_norm - f_left_norm)/(f_right_norm + f_left_norm)
...
def get_file(initdir='.', ext=''):
    ...
    root.focus_force()
    if ext=='bz2':
        filetypes = [("bz2 Files", ".bz2"), ("all_files", ".*")]
    else:
        filetypes = [("all_files", ".*"), ("binaries",".bin")]
    try:
        root.filename =  filedialog.askopenfilename(initialdir = initdir,title = "Select file", filetypes=filetypes)
        root.call('wm', 'attributes', '.', '-topmost', True)
    except FileNotFoundError:
        print('Something went wrong in the folder selection')
    root.update()
...
def get_results(initialdir = None):
    if initialdir is None:
        initialdir = os.getcwd()
    pckl = get_file(initdir = initialdir, ext='bz2')
...
def clean_outliers(array):
    """
    https://stackoverflow.com/questions/6518811/interpolate-nan-values-in-a-numpy-array
    """
    # First, I put NaN in place of all the outliers
    smoothed = smooth_profile(array)
    array[(abs(array-smoothed))>0.2*array] = np.nan
